...
io.on('connection', (socket) => { 
    ...
    /* Output a long message on the server and send it to the clients */
    ...
    /*expected payload:
     {
           ...
           'username' : the name of the user sender, 
    ...
    */
        ...
        socket.on('send_chat_message', (payload) => {
            ...
            if ((typeof username == 'undefined') || (username === null)){
                response = {};
                response.result = 'fail';
                response.message = 'client did not send a valid username to as a message source';
                socket.emit('send_chat_message_response', response);
                serverLog('send_chat_message command failed', JSON.stringify(response));
                return;
            }
        ...
        socket.on('play_token', (payload) => {
            ...
            /* Make sure the current attept is by the correct color */
            if (color !== game.whose_turn) {
                let response = {
                    result: 'fail',
                    messager: 'play_token played the wrong color. It\'s not their turn'
                }
                socket.emit('play_token_response', response);
                serverLog('play_token command failed', JSON.stringify(response));
                return;
            ...
            if (
                ...
                let response = {
                    result: 'fail',
                    messager: 'play_token played the right color, but by the wrong player'
                }
                socket.emit('play_token_response', response);
                serverLog('play_token command failed', JSON.stringify(response));
                return;
...
function adjacent_support(who, dr, dc, r, c, board) {
    ...
    /* Check to make sure that the adjacent support it is on the board */
    if (( r + dr < 0) || (r + dr > 7)) {
        return false;
    }
    if (( c + dc < 0 ) || (c + dc > 7)) {
